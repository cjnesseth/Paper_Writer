...
# Locational Deliverability Area (LDA) across all seven completed BRA years.
#
...
#
# Benchmark years: all seven completed BRAs (2021/22 through 2027/28)
#
# Outputs:
...
# Select LDAs present in at least 2 benchmark years with valid Lerner values
lda_counts <- results %>%
  filter(!is.na(lerner)) %>%
  group_by(lda) %>%
...
# Order LDAs by their latest-year Lerner, descending
lda_order <- plot_df5 %>%
  group_by(lda) %>%
  filter(delivery_year == max(delivery_year)) %>%
  ungroup() %>%
  arrange(desc(lerner)) %>%
  pull(lda) %>%
  unique()

# Pad with zero-height rows for missing LDA-year combos so all bars are equal width
plot_df5 <- plot_df5 %>%
  mutate(lda = factor(lda, levels = lda_order),
         delivery_year = factor(delivery_year, levels = BENCHMARK_YEARS)) %>%
  complete(lda, delivery_year, fill = list(lerner = 0))
...
# Write LaTeX table
n_yc <- length(year_cols)
col_spec <- paste0("l", paste(rep("c", n_yc), collapse = ""))
header_years <- paste(year_cols, collapse = " & ")
...
for (i in seq_len(nrow(tbl))) {
  ...
  row_str <- paste(c(r$lda, vals), collapse = " & ")
  tex_lines <- c(tex_lines, sprintf("    %s \\\\", row_str))
}
