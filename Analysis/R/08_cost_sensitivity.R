...
K_BASELINE  <- 3
YEARS       <- BENCHMARK_YEARS               # from 04_calibrate.R (all 7)
TABLE_YEARS <- c("2023/24", "2025/26", "2026/27")  # representative subset for LaTeX table
...
table_path <- file.path(script_dir, "../../Paper/tables/tab_cost_sensitivity.tex")

rows_tex <- character(0)
for (c_val in COST_VALS) {
  ...
  for (yr in TABLE_YEARS) {
    ...
    if (nrow(r) == 0) {
      ...
      cells <- c(cells, sprintf("\\$%d", r$p_star), sprintf("%.2f", r$lerner))
    }
  }
  rows_tex <- c(rows_tex,
                sprintf("    \\$%d & %s \\\\", c_val,
