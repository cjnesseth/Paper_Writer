...
\begin{abstract}
\singlespacing
\noindent Loudoun County, Virginia hosts roughly 200 data centers generating about \$895 million in annual tax revenue (about one-third of the county's operating budget) and funding a residential tax rate cut worth about \$2,512 per year to the median homeowner. I trace the distributional incidence across fiscal benefits, property values, and electricity rates. A \citet{SunAbraham2021} cohort-robust event study on 31,333 resale transactions (2020--2025) yields an ATT of $-1.3$ percent ($p < 0.01$), though the estimate is specification-sensitive and points toward modest negative effects in the 1--4~km range without cleanly identifying any single radius. Data-center-driven grid costs of \$168--\$444 per year per residential customer are borne statewide and addressed through rate design reform. The asymmetry across channels (large diffuse benefits and fragile evidence of small localized harm) cuts against the welfare case for the county's 2025 shift from by-right development to discretionary Special Exception review.
\end{abstract}
...
\caption[Geographic Distribution of Data Centers and Residential Transactions]{Geographic Distribution of Data Centers and Residential Transactions. Black squares are 101 data center facilities, and colored points are residential transactions, 2020--2025, classified by distance to the nearest data center: red $=$ 0--1~km, gold $=$ 1--2~km, blue $=$ 2--4~km, gray $=$ 4+~km.}
\label{fig:map}
\end{figure}
...
Figure~\ref{fig:sa} plots the cohort-robust event study coefficients using permit year treatment timing. Pre-treatment coefficients at event times $-3$ and $-2$ are small and not individually significant, but a joint Wald test rejects parallel pre-trends at the 5~percent level (TWFE $F(2,49)=3.23$, $p=0.048$; SA $F(2,50)=3.81$, $p=0.029$; Appendix Table~\ref{tab:appx_pretrends}). The test does not reject at the 1~percent level, and the two leads point in opposite directions ($-0.019$ and $+0.019$), which reads more like noise than a systematic pre-trend. The rejection at 5~percent is still a limitation of the design and should temper confidence in the post-treatment estimates. The aggregate ATT is $-0.0134$ (SE $= 0.0048$, $p = 0.008$). At the median assessed value of \$738,730, this implies a price difference of about \$9,600, a statistically significant but economically modest effect.
...
Appendix Table~\ref{tab:appx_pretrends} reports joint Wald tests on the pre-treatment lead coefficients of the TWFE and Sun-Abraham event studies. The tests are borderline at the 5~percent level and fail to reject at the 1~percent level.
...
Appendix Table~\ref{tab:appx_heterogeneity} reports interaction tests for facility-size heterogeneity and cohort-timing heterogeneity. Neither difference is statistically significant, supporting the interpretation that treatment effects are broadly homogeneous along these dimensions.
